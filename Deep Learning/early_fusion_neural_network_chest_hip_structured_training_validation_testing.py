...
def multi_source_generator(generator,
    ...
    # we should not shuffle here

    # we have multiple data generators

    # in case of shuffle, there will be mismatch between image & structured data
    ...
    gen2 = generator.flow_from_dataframe(dataframe = dataframe_imaging,
    ...
    structured_dataset_iterator = dataset.as_numpy_iterator()
    
    while True:
        X1i = gen1.next()
        X2i = gen2.next()
        X3i = structured_dataset_iterator.next()
        
        if y_col:
            yield [X1i[0], X2i[0], X3i], X1i[1]  #X1i[1] is the label
        else:
            yield [X1i, X2i, X3i]
...
def build_model(base_model_chest = None, base_model_hip = None):
    ...
    z = Dense(16, activation='relu')(input_structured)
    z = Dense(8, activation='relu')(z)
    z = Dense(4, activation='relu')(z)

    concatenated = Concatenate()([x,y,z])        
    
    q = Dense(4, activation='relu')(concatenated)

    outputs = Dense(1, activation='sigmoid')(q)
    model = Model(inputs=[base_model_chest.input,base_model_hip.input, input_structured], outputs=outputs)
    return model
...
if new_model:
    experiment_name = '30day_mortality_learning_from_chest_and_hip_and_structured_1000_40_0_structured_dataset_' + version_number+'_'+str(int(time()))
else:
    script, experiment_name = argv

filepath = '*****/hipfracture/model_weights/30day_mortality_learning/weights_' + experiment_name + ".hdf5"

classifier = 'Xception - ResNet - [16-8-4]'

experiment_notes = 'learning from chest and hip and structured simultaneously, from imagenet weights'
...
# callbacks to use during training
    ...
    y_predict_scores = model.predict(multi_source_generator(generator = test_datagen,
                                            ...
                                            img_size1 = (image_dim_chest,image_dim_chest),
                                            img_size2 = (image_dim_hip,image_dim_hip)),
                                            verbose = 1, steps = int(math.ceil(len(X_val) /batch_size)))
